...
class Dovetail:
    ...
    def MakeDovetail(self):
        # Coil cylinder 1
        self.coilcylinder1 = Part.makeCylinder(
            self.coilradius, self.coillength, 
            self.origin + FreeCAD.Vector(-self.coilxshift-self.coillength,
                                         0,
                                         self.baseheight+self.coilradius), 
        ...
        self.coilcylinder1 = self.coilcylinder1.fuse(
            Part.makeBox(self.coillength, self.upperbasewidth,
                         self.coilradius - self.coilinnerradius,
                         self.origin + FreeCAD.Vector(
                             -self.coilxshift-self.coillength,
                             -self.upperbasewidth/2,
                             self.baseheight),
              FreeCAD.Vector(0,0,1)))

        # Coil cylinder 2
        self.coilcylinder2 = Part.makeCylinder(
            self.coilradius, self.coillength, 
            self.origin + FreeCAD.Vector(self.coilxshift,
                                         0,
                                         self.baseheight+self.coilradius), 
            FreeCAD.Vector(1,0,0), 360)
        ...
        self.dovetailpolygon=Part.makePolygon([
            ...
            FreeCAD.Vector(-self.baselength/2, -self.basewidth/2, 0),
            FreeCAD.Vector(-self.baselength/2,
                           -self.upperbasewidth/2,
                           self.baseheight),
            FreeCAD.Vector(-self.baselength/2,
                           self.upperbasewidth/2,
        ...
        self.dovetailbase=self.dovetailface.extrude(
            FreeCAD.Vector(self.baselength, 0, 0))
...
class Refractor:
    ...
    def MakeRefractor(self):
        ...
        self.dewshieldcone=Part.makeCone(self.tuberadius, self.dewshieldradius, 
                 ...
                 self.origin + FreeCAD.Vector(
                     self.dewshieldxshift-self.dewshieldconelength, 0, 0), 
                 FreeCAD.Vector(1,0,0), 360)
...
class Crayford:
    ...
    def MakeCrayford(self):
        self.crayfordcylinder = Part.makeCylinder( self.crayfordcylinderradius,
            self.crayfordcylinderlength,
            self.origin + FreeCAD.Vector(self.crayfordcylinderxshift, 0, 0), 
            FreeCAD.Vector(1,0,0))
        self.crayfordcone = Part.makeCone(self.crayfordconeradius,
            self.crayfordcylinderradius, self.crayfordconelength,
            self.origin,
            FreeCAD.Vector(-1,0,0))
        self.crayfordcylinder = self.crayfordcylinder.fuse(self.crayfordcone)      

        self.crayfordbox = Part.makeBox(self.crayfordboxsize,
            2*self.crayfordcylinderradius, self.crayfordboxsize,
            self.origin + FreeCAD.Vector(self.crayfordboxxshift,
                                         -self.crayfordcylinderradius,
                                         -self.crayfordcylinderradius),
            FreeCAD.Vector(0,0,1))
        self.crayfordbox = self.crayfordbox.fuse( Part.makeCylinder(
            self.crayfordaxisradius, self.crayfordaxislength,
            self.origin + FreeCAD.Vector(
                self.crayfordboxxshift+self.crayfordboxsize/2,
                -self.crayfordaxislength/2, 
                -self.crayfordcylinderradius+self.crayfordboxsize/2),
            FreeCAD.Vector(0,1,0)))
        self.crayfordbox = self.crayfordbox.fuse( Part.makeCylinder(
            self.crayfordwheelradius, self.crayfordwheellength,
            self.origin + FreeCAD.Vector(
                self.crayfordboxxshift+self.crayfordboxsize/2,
                -self.crayfordaxislength/2, 
                -self.crayfordcylinderradius+self.crayfordboxsize/2),
        ...
        self.crayfordcylinder = self.crayfordcylinder.cut( Part.makeCylinder(
            self.crayfordcylinderinnerradius, self.crayfordcylinderlength,
            self.origin+FreeCAD.Vector(self.crayfordcylinderxshift, 0, 0), 
            FreeCAD.Vector(1,0,0)))

        self.crayfordspacer = Part.makeCylinder( self.tubespacerradius,
            self.tubespacerlength, self.origin, FreeCAD.Vector(1,0,0))
        self.crayfordspacer = self.crayfordspacer.cut( Part.makeCylinder(
            self.crayfordcylinderradius, self.tubespacerlength, self.origin, 
            FreeCAD.Vector(1,0,0)))

        self.crayfordtube = Part.makeCylinder(self.crayfordtuberadius,
            self.crayfordtubelength, self.origin + FreeCAD.Vector(
                self.crayfordcylinderxshift+self.crayfordtubexshiftmin, 0, 0), 
            FreeCAD.Vector(1,0,0))
        self.crayfordtube=self.crayfordtube.cut(Part.makeCylinder(
            self.crayfordtubeinnerradius, self.crayfordtubelength,
            self.origin+FreeCAD.Vector(
                self.crayfordcylinderxshift+self.crayfordtubexshiftmin, 0, 0), 
            FreeCAD.Vector(1,0,0))) 
    ...
    def draw(self, cylindermat, tubemat, spacermat):
        ...
        self.rotationcrayford.rotation.setValue(pivy.coin.SbVec3f(1,0,0),
                                                math.radians(0.0))
