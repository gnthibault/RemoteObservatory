        ...
        for tabitem in iter(self.config):
            ...
            for debugvar in iter(self.config[tabitem]):
                if self.config[tabitem][debugvar]['type'] == 'lcd':
                    varlcds.append((debugvar, debugvar,
                                    int(self.config[tabitem][debugvar]['size']),
    ...
    def process_json(self, json_data, tcp_handler):
        ...
        for v in iter(self.values):
            ...
            if (v == 'ra_position'):
                ...
                self.manualdock.raangleSPV.changeValue(
                    float(360 * angle) / 0x1C2000)
    ...
    def readDatas(self):        
        ...
        while (self.bytestoread - self.currentread) <= rest:
            self.currentData.append(
                datas.left(self.bytestoread - self.currentread))
...
def acceptClient(server, process_json):
    ...
    if server.currentClient == None:
        server.currentClient = TCPHandler(server.nextPendingConnection(),
            server, process_json)
