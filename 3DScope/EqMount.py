...
class Baseholder:
    ...
    def MakeBaseholder(self):
        # Base cylinder
        self.basecylinder = Part.makeCylinder(self.baseradius, self.baseheight,
            self.origin, FreeCAD.Vector(0,0,1), 360)
        self.base = self.basecylinder.makeFillet(
            self.basefillet, [self.basecylinder.Edges[0]])

        # Holder box
        self.holderbox = Part.makeBox(self.holderwidth, self.holderlength,
            self.holderheight, self.origin + FreeCAD.Vector(self.holderxshift,
                                                           self.holderyshift,
                                                           self.baseheight),
           FreeCAD.Vector(0,0,1))
        self.holderbox = self.holderbox.makeFillet((self.holderwidth/2)-0.5,
            [self.holderbox.Edges[1], self.holderbox.Edges[5]])
        self.holderspacer = Part.makeBox(self.holderwidth,
            self.holderspacerlength, self.holderspacerheight, self.origin +
            FreeCAD.Vector(self.holderxshift,
                           -(self.holderspacerlength / 2), 
                           (self.baseheight + self.holderheight) -
                               self.holderspacerheight),
            FreeCAD.Vector(0,0,1))
        self.holderbox = self.holderbox.cut(self.holderspacer)
        self.holderaxis = Part.makeCylinder(self.holderaxisradius,
            self.holderlength, self.origin + FreeCAD.Vector(
                self.holderxshift + (self.holderwidth/2),
                -self.holderlength/2, 
...
class Rahousing:
    ...
    def MakeRahousing(self):
        self.latitudeaxis = Part.makeCylinder(self.latitudeaxisradius,
            self.latitudeboxaxislength,
            self.origin + FreeCAD.Vector(0, -self.latitudeboxaxislength/2, 0),
            FreeCAD.Vector(0,1,0), 360)
        self.latitudeaxishole = Part.makeCylinder(self.latitudeaxisholeradius,
            self.latitudeboxlength,
            self.origin + FreeCAD.Vector(0, -self.latitudeboxlength/2, 0),
            FreeCAD.Vector(0,1,0), 360)
        self.latitudebox = Part.makeBox(self.latitudeboxwidth,
            self.latitudeboxlength, self.latitudeboxheight,
            self.origin + FreeCAD.Vector(self.latitudeboxxshift,
                                         self.latitudeboxyshift,
                                         self.latitudeboxzshift), 
            FreeCAD.Vector(0,0,1))
        self.latitudebox=self.latitudebox.cut(Part.makeCylinder(
            self.latitudeboxaxisradius,
            (self.latitudeboxlength - self.latitudeboxaxislength)/2,
            self.origin + FreeCAD.Vector(0, self.latitudeboxaxislength/2, 0),
            FreeCAD.Vector(0,1,0), 360)) 
        self.latitudebox = self.latitudebox.cut( Part.makeCylinder(
            self.latitudeboxaxisradius,
            ...
            self.origin+FreeCAD.Vector(0, -self.latitudeboxlength/2, 0),
            FreeCAD.Vector(0,1,0), 360)) 
        self.latitudebox=self.latitudebox.cut(Part.makeBox(
            self.latitudeboxaxisradius,
            (self.latitudeboxlength-self.latitudeboxaxislength)/2,
            self.latitudeboxaxisradius, 
            self.origin+FreeCAD.Vector(0,
                                       -self.latitudeboxlength/2,
                                       -self.latitudeboxaxisradius),
            FreeCAD.Vector(0,0,1)))
        self.latitudebox=self.latitudebox.cut( Part.makeBox(
            self.latitudeboxaxisradius,
            (self.latitudeboxlength-self.latitudeboxaxislength)/2,
            self.latitudeboxaxisradius, 
            self.origin+FreeCAD.Vector(0,self.latitudeboxaxislength / 2,
                                       -self.latitudeboxaxisradius),
            FreeCAD.Vector(0,0,1)))
        ...
        self.raaxis = Part.makeCylinder(self.rahousingradius,
            self.rahousinglength,
            self.origin + FreeCAD.Vector(self.rahousingxshift,
                                         0,
                                         self.raaxisshift),
            FreeCAD.Vector(1,0,0), 360)
        self.racone = Part.makeCone(self.raconeradius, self.rahousingradius,
            self.raconelength,
            self.origin + FreeCAD.Vector(self.rahousingxshift-self.raconelength,
                                         0,
                                         self.raaxisshift),
           FreeCAD.Vector(1,0,0), 360)
        self.racone = self.racone.fuse(Part.makeCylinder(self.raconeradius, 10,
            self.origin + FreeCAD.Vector(
                self.rahousingxshift-self.raconelength-10,
                0, 
                self.raaxisshift),
            FreeCAD.Vector(1,0,0), 360))
        self.raaxis=self.raaxis.fuse(self.racone)
        self.raaxis=self.raaxis.cut(Part.makeCylinder(self.raaxisradius,
            self.rahousinglength+self.raconelength+10,
            self.origin + FreeCAD.Vector(
                self.rahousingxshift-self.raconelength-10,
                0,
                self.raaxisshift),
...
class Raaxis:
    ...
    def MakeRaaxis(self):
        ...
        self.racoord = Part.makeCylinder(self.racoordcircleradius,
            self.racoordcirclelength,
            self.origin+FreeCAD.Vector(-120, 0, 0),
            FreeCAD.Vector(1,0,0), 360)
        self.racylinder = Part.makeCylinder(self.racylinderradius,
            self.racylinderlength,
            self.origin+FreeCAD.Vector(0, 0, 0), FreeCAD.Vector(1,0,0), 360)
...
class Dehousing:
    ...
    def MakeDehousing(self):
        self.debase = Part.makeCylinder(self.debasecylinderradius,
            self.debasecylinderlength,
            self.origin+FreeCAD.Vector(0, 0, 0), FreeCAD.Vector(1,0,0), 360)
        self.desphere = Part.makeSphere(self.desphereradius,
            self.origin+FreeCAD.Vector(self.despherexshift,0,0),
            FreeCAD.Vector(1,0,0), self.desphereangle, 90, 360)
        self.debase=self.debase.fuse(self.desphere)

        self.debox = Part.makeBox(self.deboxwidth, self.deboxlength,
            self.deboxheight,
            self.origin + FreeCAD.Vector(3,
                                         -self.deboxlength / 2,
                                         self.deboxzshift),
            FreeCAD.Vector(0,0,1))
        self.debox=self.debox.makeFillet(self.deboxlength/2 - 0.5,
            [self.debox.Edges[4],self.debox.Edges[6]])
        self.decone=Part.makeCone(self.deboxlength/2 , self.decylinderradius,
            self.deconelength,
            self.origin + FreeCAD.Vector(self.deaxisxshift,
                                         0,
                                         self.deboxheight + self.deboxzshift),
             FreeCAD.Vector(0,0,1), 310)
        self.decone.rotate(self.origin +
            FreeCAD.Vector(self.deaxisxshift,
                           0,
                           self.deboxheight + self.deboxzshift),
            FreeCAD.Vector(0,0,1), -155)
        self.debox=self.debox.fuse(self.decone)

        self.decylinder = Part.makeCylinder(self.decylinderradius,
            self.decylinderlength, self.origin + FreeCAD.Vector(
                self.deaxisxshift,
                0,
                self.deboxheight + self.deboxzshift + self.deconelength), 
...
class Deaxis:
    ...
    def MakeDeaxis(self):
        ...
        self.decoord = Part.makeCylinder(self.decoordcircleradius,
            self.decoordcirclelength,
            self.origin + FreeCAD.Vector(0, 0, -120),
            FreeCAD.Vector(0,0,-1), 360)
        self.decylinder = Part.makeCylinder(self.decylinderradius,
            self.decylinderlength,
            self.origin + FreeCAD.Vector(0, 0, 0), FreeCAD.Vector(0,0,1), 360)
        self.decylinder = self.decylinder.fuse(Part.makeCylinder(
            self.decylinderradius, self.demountholderlength,
            self.origin + FreeCAD.Vector(0, 0, self.decylinderlength),
            FreeCAD.Vector(0,0,1), 360))
        self.decylinder=self.decylinder.cut(Part.makeBox(
            self.decylinderradius*2, self.demountholderwidth,
            self.demountholderlength,
            self.origin + FreeCAD.Vector(-self.decylinderradius, 
                                         -self.demountholderwidth/2,
                                         self.decylinderlength), 
            FreeCAD.Vector(0,0,1)))
        ...
        self.decw = Part.makeCylinder(self.decwaxisradius, self.decwaxislength,
            self.origin + FreeCAD.Vector(0, 0, -120),
            FreeCAD.Vector(0,0,-1), 360)
        self.decw = self.decw.fuse(Part.makeCylinder(self.decwscrewradius,
            self.decwscrewlength,
            self.origin + FreeCAD.Vector(0, 0, -120-self.decoordcirclelength),
            FreeCAD.Vector(0,0,-1), 360))
        self.decw = self.decw.fuse(Part.makeCone(self.decwscrewradius,
            self.decwscrewconeradius, self.decwscrewconelength,
            self.origin + FreeCAD.Vector(
                0,
                0,
                -120 - self.decoordcirclelength - self.decwscrewlength), 
            FreeCAD.Vector(0,0,-1), 360))
...
class EQ5:
    ...
    def Makemount(self):
        ...
        self.rahousingorigin = self.origin + FreeCAD.Vector(
            self.baseholder.holderxshift+(self.baseholder.holderwidth/2),
            0,
            self.baseholder.baseheight + self.baseholder.holderheight -
                (self.baseholder.holderwidth/2))
        self.rahousing=Rahousing(self.rahousingorigin)
        self.rahousing.MakeRahousing()

        self.raaxisorigin = self.rahousingorigin + FreeCAD.Vector(
            self.rahousing.rahousingxshift + self.rahousing.rahousinglength,
            0,
            self.rahousing.raaxisshift)
