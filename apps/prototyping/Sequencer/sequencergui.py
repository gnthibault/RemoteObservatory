...
class GuiLoop():
    ...
    def __init__(self, gps_coord, mount, observatory, serv_time, indi_client):
    ...
    def run(self):
        ...
        # is ready
        self.acquisition_thread = ObservationRun(
            self.mount, self.observatory, self.indi_client)
        # Connect planner with acquisition thread
        planner.schedule_ready_trigger.connect(self.launch_acquisition_thread)
    ...
    #@pyqtSlot(object)
    def launch_acquisition_thread(self, schedule):
        self.acquisition_thread.schedule = schedule
...
class ObservationRun(QThread):
    ...
    def __init__(self, mount, observatory, indi_client):
    ...
    def run(self):
        ...
        s = ScheduleSequencer(camera=camera, filter_wheel=filter_wheel,
                              ...
                              use_auto_flat=True, schedule=self.schedule)
        s.build_sequence()
        s.start_sequence()
...
if __name__ == "__main__":
    ...
    indi_client = Indi3DSimulatorClient(None)
    indi_client.connect()
