...
#         self.axes = plt.gca()
    ...
    def get_table_data(self, data_file):
        ...
        if data_file is not None:
            ...
            from utils.database import DB, FileDB

            db = DB()
            if isinstance(db, FileDB):
                with open(db.get_file(collection='weather')) as f:
                    d = deque(f.readlines(), maxlen=10000)
                ...
                #table = Table.from_pandas(pd.read_csv(db.get_file('weather'), parse_dates=True))
            else:
                # -------------------------------------------------------------------------
    ...
    def get_twilights(self, config=None):
        ...
        if config is None:
            from utils.config import load_config as pocs_config
    ...
    def save_plot(self, plot_filename=None):
        ...
        if plot_filename is None:
            ...
            plot_filename = os.path.join(
                self.config['directories']['images'], plot_filename)
...
def moving_average(interval, window_size):
    ...
    return np.convolve(interval.astype(np.float64), window, 'same')
...
def moving_averagexy(x, y, window_size):
    ...
    window = np.ones(int(window_size)) / float(window_size)
    yma = np.convolve(y.astype(np.float64), window, 'valid')
    xma = x[2 * nxtrim:]
