...
from helper.PyIndi import PyIndi
...
class IndiDevice(Base, device):
    ...
    def connect_device(self, force_reconnect=False):
        ...
        if force_reconnect:
            self.set_switch('CONNECTION', ['DISCONNECT'], sync=True, timeout=self.defaultTimeout)
    ...
    def set_switch(self, name, on_switches=[], off_switches=[],
        ...
        self.set_and_send_switchvector_by_element_name(name, on_switches=on_switches, off_switches=off_switches)
        if sync:
           self.wait_for_vector_light(name, timeout=timeout)
